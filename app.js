const STORAGE_KEY = "christendomBudgetWebSettings";
...
function fillSettingsForm() {
  ...
  els.folderInput.value = settings.folder || "budget-data";
...
async function githubPutBytes(filename, bytes, sha) {
  const body = {
    message: "Save Christendom Budget database",
...
function renderDashboard() {
  ...
  els.spendingValue.className = data.planned > 0 ? (data.spending > data.planned ? "negative" : "positive") : "";
...
async function resetBudgetFromDefaults() {
  ...
  state.month = month;
  await saveAfterChange("Reset " + rows.length + " budget line(s) from defaults.");
}
...
function bindEvents() {
  ...
  els.settingsForm.addEventListener("submit", async function (event) {
    ...
    saveSettings({
      ...
      folder: els.folderInput.value.trim() || "budget-data",
